        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="rounded-3xl border border-stone-100 bg-white/90 p-6 shadow-lg">
                <div class="grid gap-4 sm:grid-cols-3 text-sm">
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-2xl font-semibold text-stone-900">INR {{ number_format($order->total, 2) }}
                        </p>
                        <span
                            class="mt-2 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
            ...
            <div class="rounded-3xl border border-stone-100 bg-white/90 p-6 shadow-lg">
                ...
                <div class="mt-4 space-y-3">
                    ...
                    <div
                        class="flex flex-wrap items-center justify-between gap-3 text-sm border-b border-stone-100 pb-3">
                        <span class="text-stone-700">{{ $item->product->name ?? 'Product' }} x
                            {{ $item->quantity }}</span>
            ...
            <form method="POST" action="{{ route('admin.orders.update', $order) }}"
                class="rounded-3xl border border-stone-100 bg-white/90 p-6 shadow-lg space-y-3">
                @csrf
                @method('PATCH')
                <label class="block text-sm font-medium text-stone-700">Update status</label>
                <select class="w-full rounded-xl border-stone-200 focus:border-emerald-400 focus:ring-emerald-200"
                    name="status">
                ...
                <button class="rounded-full bg-emerald-600 px-6 py-2 text-white shadow-sm hover:bg-emerald-500"
